...
/* MySQL does NOT support MINUS or EXCEPT */
...
-- This lists the invoices with an invioce_total 
-- above the average invoice price 
...
USE ap;
-- Vendors with at least one invoice
-- Solution1
SELECT DISTINCT v.vendor_id, vendor_name, vendor_state
...
SELECT vendor_id, vendor_name, vendor_state
FROM vendors
WHERE EXISTS -- vendor_id IN (select distinct vendor_id from invoices)
    (SELECT * 
     FROM invoices
     WHERE invoices.vendor_id = vendors.vendor_id);
